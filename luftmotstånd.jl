...
begin
	...
	scatter(x, y, grid=true, label="drag", markersize=5, markercolor=:blue,
		...
		xlims =(0, 4000), ylims=(0, 1050), fmt=:png)
...
begin
	scatter([0:dt:t], y, grid=true, label="drag", markersize=5, 
		markercolor=:blue, xlabel="t (s)", ylabel="höjd (m)",
		xlims =(0, 15), ylims=(0, 1050), fmt=:png)
	scatter!([0:dt:t], y1, label="no drag", markersize=5, markercolor=:red)
end

# ╔═╡ f7c8614e-6ca9-11eb-3437-93ea978f38b6
begin
	scatter([0:dt:t], x, grid=true, label="paket", markersize=5,
		markercolor=:blue, xlabel="t (s)", ylabel="längd (m)",
		xlims =(0, 15), ylims=(0, 4000), fmt=:png)
